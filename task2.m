...
sim_media_APD_data = zeros(1,4);
sim_media_APD_voip = zeros(1,4);

theo_media_APD_data = zeros(1,4);
theo_media_APD_voip = zeros(1,4);
theo_media_APD_data_2 = zeros(1,4);
theo_media_APD_voip_2 = zeros(1,4);
for i= 1:length(n)
    for it= 1:N
       [PL_data(it), PL_voip(it), APD_data(it), APD_voip(it), MPD_data(it), MPD_voip(it), TT(it)] = Simulator4(lambda,C,f,P,n(i));
        [theo_media_APD_voip_2, theo_media_APD_data_2] =  Theo_Avg_Delay_MG1(lambda, C, n(i));
    end
    sim_media_APD_data(i) = mean(APD_data);
    sim_media_APD_voip(i) = mean(APD_voip);
    theo_media_APD_data(i) = mean(theo_media_APD_data_2);
    theo_media_APD_voip(i) = mean(theo_media_APD_voip_2);
end
figure(5);
bar(n,[sim_media_APD_data; theo_media_APD_data]);
hold on
grid on
title('Average Data Packet Delay');
...
figure(6);
bar(n,[sim_media_APD_voip; theo_media_APD_voip]);
hold on
grid on
title('Average VoIP Packet Delay');
legend('Simulation','Theoretical', 'location', 'northwest');
